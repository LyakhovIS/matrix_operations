...
void InputMatrix(int n, int k, double *a, double *b, int first_row, int max_rows){          // матрицы в процессах будем хранить поблочно, строка i
    int i, j;                                                                               // в процессе будет строкой i + first_row в большой матрице
    double x = 0;
...
int fileInputMatrix(int n, char *name, double *a, double *b, double *x, int my_rank, int p){
    ...
    if (my_rank == 0){                                                                      // изначально из файла читаем первым процессом
        IN = fopen(name, "r");
        if (IN == NULL){                                                                    // если файл не открывается, завершаем 1 процесс
        ...
        for (i = 0; i < n; ++i){
            for (j = 0; j < n; ++j){
                if (fscanf(IN, "%lf", &x[i * n + j]) != 1){                                 // читаем всю матрицу из файла, сохраняем в буффер
                    fclose(IN);
                    err = 2;                                                                // если чисел меньше, чем нкжнл, завершаем 1 процесс
        ...
        for (j = first_row; j <= last_row; ++j){                                            // заполняем матрицу и вектор нулевого процесса
            for (k = 0; k < n; ++k){
                a[j * n + k] = x [j * n + k];
                if (!(k % 2))
                    z += x[j * n + k];
        ...
        MPI_Bcast(&err, 1, MPI_INT, 0, MPI_COMM_WORLD);                                     // отсылаем всем процессам ошибку 0, если все хорошо
        for (j = 1; j < p; ++j)                                                             // отсылаем всем остальным процессам буффер
            MPI_Send(x, n * n, MPI_DOUBLE, j, 0, MPI_COMM_WORLD);

    } else{
        MPI_Bcast(&err, 1, MPI_INT, 0, MPI_COMM_WORLD);                                     // принимаем ненулевыми процессами ошибки
        if (err == 1)                                                                       // завершаем процессы в соответствии с ошибками
            return -1;
        if (err == 2)
            return -2;

        MPI_Recv(x, n * n, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);                      // принимаем буффер процессами

        for (j = first_row; j <= last_row; ++j){                                            // заполняем матрицы процессов, учитывая, что они сдвинуты
...
void OutputMatrix(int l, int n, int m, double *a, double *x, int my_rank, int p){
	...
	if (l == n){
        if (my_rank != 0){                                                                  // записываем матрицы процессов в буффер
            for (i = 0; i < max_rows; ++i){
                for (j = 0; j < en; ++j)
                    x[i * n + j] = a[i * n + j];
            }

            MPI_Send(&max_rows, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);                          // посылаем количество строк в данном процессе
            MPI_Send(x, en*max_rows, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);                     // и буффер нулевому процессу

        } else{

            for (i = 0; i < em; ++i){                                                       // нулевым процессом сначала печатаем матрицу нулевого процесса
                for (j = 0; j < en; ++j)
